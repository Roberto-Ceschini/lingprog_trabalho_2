...
# Análise de Grafos em C++

## Descrição

Sistema desenvolvido em C++ para representação e análise de grafos ponderados utilizando matriz de adjacência e algoritmos clássicos de teoria dos grafos.

O programa realiza a leitura de um arquivo `.txt` contendo as conexões entre vértices e, a partir dessas informações, monta automaticamente o grafo utilizado nas análises.

O sistema permite:

- construção automática do grafo a partir de arquivo;
- geração da matriz de adjacência;
- cálculo do grau dos vértices;
- verificação de conectividade do grafo;
- cálculo de menores caminhos utilizando o algoritmo de Dijkstra;
- cálculo de centralidade dos vértices.
---
